...
const getRequestIp = require('../shared/better-auth.BgkOX1Ei.cjs');
require('defu');
...
function createApiKey({
  ...
  return account.createAuthEndpoint(
    ...
    async (ctx) => {
      ...
      const hash$1 = await hash.createHash("SHA-256").digest(key);
      const hashed = base64.base64Url.encode(hash$1, {
        padding: false
      });
      let start = null;
      ...
      if (metadata) {
        const parseMetadata = schema.parseInputData(
          data,
          apiKeySchema({
      ...
      const apiKey = await ctx.context.adapter.create({
        model: schema$1.apikey.modelName,
        data
      });
      return ctx.json({
        ...apiKey,
